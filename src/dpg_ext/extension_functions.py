...
def create_confirm_popup(text: str = "", on_cancel: Callable = None, on_confirm: Callable = None, title: str = "Confirmation", width=350,
                            always_center: bool = True, confirm_text="Confirm", cancel_text="Cancel", close_on_confirm=True, close_on_cancel=True, confirm_kwargs={}, cancel_kwargs={}):
    """
    Creates a modal window with supplied text and a cancel and confirm button with settable callbacks
    """
    return PopupWindow(title=title, width=width, always_center=always_center).set_confirm_box(text=text, title=title, on_cancel=on_cancel, on_confirm=on_confirm, confirm_text=confirm_text, cancel_text=cancel_text, close_on_confirm=close_on_confirm, close_on_cancel=close_on_cancel, confirm_kwargs=confirm_kwargs, cancel_kwargs=cancel_kwargs)